...
function DatePicker({ selectedDate, onDateSelect, onSelect }) {
  ...
  useEffect(() => {
    ...
    if (idx < 0) return
    const button = scrollRef.current.children[idx + 1]
    ...
    <section className="w-full max-w-full overflow-hidden rounded-2xl border border-gray-100 bg-white p-3 sm:p-4">
      ...
      <div className="relative w-full max-w-full overflow-hidden">
        ...
        <div
          ...
          style={{ WebkitOverflowScrolling: 'touch', scrollPaddingInline: 'calc(50% - 40px)' }}
        >
          <div className="w-[calc(50%_-_40px)] flex-shrink-0" aria-hidden />
          {days.map((day) => {
            ...
            return (
              <motion.button
                ...
                className={`snap-center w-[80px] flex-shrink-0 rounded-2xl border px-2 py-2.5 text-center transition-all duration-200 ${
                  isSelected
