...
export default function Cart() {
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 py-12">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
          ...
          <p className="text-gray-500 mb-6">Parcourez nos boutiques pour ajouter des articles</p>
          <Link
            to="/salons?businessType=BOUTIQUE"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gray-900 text-white font-semibold hover:bg-gray-800 transition"
          >
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Votre panier</h1>
      {salon?.name && <p className="text-sm text-gray-500 mb-5">Boutique : {salon.name}</p>}
        ...
        {items.map((c, idx) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm font-semibold text-amber-600 mt-1">{formatPrice(c.product.price)}</p>
            </div>
      ...
      <div className="bg-gray-50 rounded-2xl p-4 mb-4">
        ...
        <p className="text-xs text-gray-500 mt-1">{items.reduce((s, c) => s + c.quantity, 0)} article(s)</p>
      </div>

      <button
        onClick={handleCheckout}
        className="w-full py-3.5 rounded-xl bg-gradient-to-r from-amber-600 via-amber-500 to-amber-600 text-white font-semibold shadow-lg hover:shadow-xl transition-all"
      >
      ...
      <button
        ...
        className="w-full mt-3 py-2.5 rounded-xl border border-gray-200 text-gray-600 text-sm font-medium hover:bg-gray-50 flex items-center justify-center gap-2"
      >
